...
class StyleTransferer {
    ...
    fileprivate init(
        ...
        if #available(iOS 13.0, *) {
            self.inputSize = Constants.inputImageSize512
        }else{
            self.inputSize = Constants.inputImageSize
        }
    }
    ...
    func runStyleTransfer(style styleImage: UIImage,
        ...
        tfLiteQueue.async {
            ...
            guard let cgImage = self.postprocessImageData(data: outputTensor.data, size: self.inputSize) else {
                DispatchQueue.main.async {
    ...
    private func postprocessImageData(data: Data,
        ...
        guard
            ...
            let cgImage = CGImage(
                ...
                bitsPerComponent: 8,
                bitsPerPixel: 32,
                bytesPerRow: MemoryLayout<UInt8>.size * 4 * Int( self.inputSize.width),
                space: colorSpace,
...
private enum Constants {
    ...
    static let inputImageSize512 = CGSize(width: 512, height: 512)
